...
export async function captureClickDetails(request, config, geoIpResolver) {
    ...
    return {
        ...
        requestPath: captureRequestPath(request.originalUrl || request.url),
        httpVersion: normalizeHttpVersion(request.httpVersion),
        requestHeaders: captureRequestHeaders(request.headers),
    };
}
...
export function captureRequestHeaders(headers) {
    ...
    for (const [rawName, rawValue] of entries) {
        ...
        const value = joined.replace(/[\u0000-\u001f\u007f]/g, " ").slice(0, MAX_HEADER_VALUE_LENGTH);
...
export function sanitizeRequestHeaders(headers, hideSensitiveHeaders = true) {
    const captured = captureRequestHeaders(headers);
    if (!hideSensitiveHeaders) return captured;

    return Object.fromEntries(
        Object.entries(captured).map(([name, value]) => {
            if (SENSITIVE_HEADER_PATTERN.test(name)) return [name, "[redacted]"];
            if (CLIENT_IP_HEADERS.has(name)) return [name, "[redacted for privacy]"];
            return [name, value];
        }),
    );
}
...
function captureRequestPath(value) {
    if (typeof value !== "string" || !value.startsWith("/")) return "/";
    return value.slice(0, 2_048);
}

export function sanitizeRequestPath(value, hideSensitiveHeaders = true) {
    if (typeof value !== "string" || value.length === 0) return "";
    const captured = captureRequestPath(value);
    if (!hideSensitiveHeaders) return captured;

    try {
        const parsed = new URL(captured, "http://analytics.invalid");
...
export function buildAnalytics(
    ...
    const hideSensitiveHeaders = options.hideSensitiveHeaders !== false;
    ...
    return {
        ...
        sensitiveDataHidden: hideSensitiveHeaders,
        ...
        recentClicks: recentEvents.slice(0, recentLimit).map((event) => ({
            ...
            ipAddress: hideSensitiveHeaders
            ...
            request: {
                ...
                path: sanitizeRequestPath(event.requestPath || "", hideSensitiveHeaders),
                httpVersion: event.httpVersion || "",
                headers:
                    event.requestHeaders && typeof event.requestHeaders === "object"
                        ? sanitizeRequestHeaders(event.requestHeaders, hideSensitiveHeaders)
                        : {},
            },
        })),
    };
